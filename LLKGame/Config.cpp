...
void CConfig::Load()
{
	...
	::GetPrivateProfileString(_T("THEME"), _T("ElementPath"),DEFAULTELEMENT, m_strElemPath.GetBufferSetLength(MAXFILEPATHLENGTH), MAXFILEPATHLENGTH, CONFIGFILEPATH);
	m_strElemPath.ReleaseBuffer();
	::GetPrivateProfileString(_T("THEME"), _T("MaskPath"),DEFAULTMASK, m_strMaskPath.GetBufferSetLength(MAXFILEPATHLENGTH), MAXFILEPATHLENGTH, CONFIGFILEPATH);
	m_strMaskPath.ReleaseBuffer();
	//±≥æ∞≈‰÷√
	m_nBGPStyle= ::GetPrivateProfileInt(_T("PICTURE"), _T("PictureStyle"), 0, CONFIGFILEPATH);
	::GetPrivateProfileString(_T("PICTURE"), _T("PicturePath"), DEFAULTPICTURE, m_strBGPPath.GetBufferSetLength(MAXFILEPATHLENGTH), MAXFILEPATHLENGTH, CONFIGFILEPATH);
